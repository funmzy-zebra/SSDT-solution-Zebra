...
CREATE PROCEDURE [dbo].[BQ1_Manual_LoadTableByDate_ETL]
/*** Description
...
SET @endDate = CONVERT(DATE, CONVERT(DATETIME,DATEADD(day, 1, @startDate)));
...
SET @shell_cmd = 'bq load --replace --field_delimiter="' + @delimiter + '" --quote="" ' 
				+ @bigQueryDB + '.' + @bigQueryTable + '$' + replace( @startDate,'-','') 
				+ ' ' + @gcpPath +  @filename; 
PRINT @shell_cmd;
EXEC sp_nextgen_etl_run_cmd @shell_cmd;
