...
CREATE PROCEDURE [dbo].[BQ_LoadTable_ETL_ByQuery]
/*** Description
...
SET @shell_cmd =  'bcp.exe "'+@query+'" queryout "' 
				+ @localPath + @sourceTable + '.csv" -c -t"' + @delimiter + '" -T';
PRINT @shell_cmd;
EXEC sp_nextgen_etl_run_cmd @shell_cmd;


SET @shell_cmd = 'gsutil cp ' + @localPath + @sourceTable + '.csv '+ @gcpPath
PRINT @shell_cmd;
EXEC  sp_nextgen_etl_run_cmd @shell_cmd;
...
SET @shell_cmd = 'gsutil rm ' + @gcpPath+@sourceTable + '.csv '
PRINT @shell_cmd;
EXEC  sp_nextgen_etl_run_cmd @shell_cmd;
