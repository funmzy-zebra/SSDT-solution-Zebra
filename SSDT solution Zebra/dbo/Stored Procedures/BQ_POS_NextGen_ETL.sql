...
CREATE PROCEDURE [dbo].[BQ_POS_NextGen_ETL] 
...
BEGIN
	...
	SET NOCOUNT ON;
           ...
           EXEC sp_nextgen_etl_run_cmd 'cd C:\Program Files (x86)\Microsoft SQL Server\140\Tools\Binn\';
	       EXEC sp_nextgen_etl_run_cmd 'gcloud auth activate-service-account sobeys-nextgen-prd-etl@sobeys-nextgen-prd.iam.gserviceaccount.com --key-file=C:\GCPKey\sobeys-nextgen-prd-15c70ef68c9e.json'    
		   
		 --  EXEC sp_nextgen_etl_run_cmd 'gcloud config set project sobeys-gcp'
		 ...
		 -- exec [ProfitectDB_Sobeys]..[BQ_LoadTableToBucket_ETL] 'Staging_Sobeys','Master_Bags_upc', '\\SOBEYS-PRD-DB\X$\Profitect\BQ_Data\MasterData','gs://sobeys-nextgen-dev1-bucket/external_csv/','|';	
		  ...
		  Declare @dbName nvarchar(1000)= N'Staging_Sobeys'
			...
			*/
			...
			SELECT @i =  MIN(ID) from NextGenETL..MasterTables_ForNextGen
			SELECT @id = MAX(ID) from NextGenETL..MasterTables_ForNextGen
